...
class InpaintingTrain_ldm(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image_address='image'+mask_address[4:]
        
        mask = Image.open(self.data_root+"/masks/"+mask_address).convert("L").resize((256,256))
        mask = np.array(draw_rectangle_over_mask(mask))
        mask = np.expand_dims(mask, axis=2)
        
        
        
        image = np.array(Image.open(self.data_root+"/images/"+image_address).convert("RGB").resize((256,256)))
        image = image.astype(np.float32) / 255.0#
        #image = image[None].transpose(0,3,1,2)
        image = torch.from_numpy(image)
